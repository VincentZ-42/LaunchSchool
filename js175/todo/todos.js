...
app.get("/lists/:todoListId", (req, res, next) => {
  ...
  // the plus in front of todoListId is to coerce the number into a string
...
app.post("/lists/:todoListId/complete_all", (req, res, next) => {
  ...
  if (!todoList) {
    ...
    req.flash("success", `All tasks has been completed!`);
    res.redirect(`/lists/${todoListId}`);
  }
});
...
app.post("/lists/:todoListId/todos",
  [
    body("todoTitle")
      ...
      .isLength({ max: 100 })
      .withMessage("todo name must be between 1 and 100 characters.")
  ],
  (req, res, next) => {
    let todoListId = req.params.todoListId;
    let todoList = loadTodoList(+todoListId, req.session.todoLists);
    if (!todoList) {
      ...
      if (!errors.isEmpty()) {
        ...
        req.flash("success", `${title} has been created.`);
        res.redirect(`/lists/${todoListId}`);
      }
    }
  }
...
app.post("/lists/:todoListId/destroy", (req, res, next) => {
  ...
  if (index === -1) {
    ...
    req.flash("success", `${title} list deleted.`);
    res.redirect("/lists");
  }
});
